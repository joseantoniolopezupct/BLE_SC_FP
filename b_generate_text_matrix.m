...
%% Generate a vector with filenames in the test folder
%%%%%%%%%%%%%%%%%
% Change the folder for each test day
%%%%%%%%%%%%%%%%%
test_folder = 'Test_day_15_with_furnitures_05122023/';

filename = {};
files =dir(test_folder);
i=1;
...
day = strsplit(test_folder,'_');
day = day{3};

%% Load RSSI from files to generate the test matrix
% We generate a different row of the test matrix per each file
for z=1:length(filename)
    ...
    dataraw = importdata(strcat(test_folder,filename{z}), ';' );
    ...
    port1_UC = find( strcmp(dataraw.textdata(:,2),'P1'));
    port2_UC = find( strcmp(dataraw.textdata(:,2),'P2'));
    port3_UC = find( strcmp(dataraw.textdata(:,2),'P3'));
    port4_UC = find( strcmp(dataraw.textdata(:,2),'P4'));
    port5_UC = find( strcmp(dataraw.textdata(:,2),'P5'));
    port6_UC = find( strcmp(dataraw.textdata(:,2),'P6'));
    port7_UC = find( strcmp(dataraw.textdata(:,2),'P7'));
    port8_UC = find( strcmp(dataraw.textdata(:,2),'P8'));
    
    data_port1_UC = dataraw.data(port1_UC,:);
    data_port2_UC = dataraw.data(port2_UC,:);
    data_port3_UC = dataraw.data(port3_UC,:);
    data_port4_UC = dataraw.data(port4_UC,:);
    data_port5_UC = dataraw.data(port5_UC,:);
    data_port6_UC = dataraw.data(port6_UC,:);
    data_port7_UC = dataraw.data(port7_UC,:);
    data_port8_UC = dataraw.data(port8_UC,:);
    ...
    %We need aggregate the 3 RSSI from channels 37, 38 and 39
    %We use this bucle iteration
    w = 3;
    v = 1;

    % 10 sampleas per point
    for n = 1:10 
        %SCFP test vector for LWA --> Ports from #1 to #4 and different
        %channels
        LWA_SC_name_vector = sprintf('x_%d_y_%d_n_%d_LWA_SC', X, Y, n);
        LWA_SC_RSSI_vector = [data_port1(index_data_port1_37(n),2), data_port2(index_data_port2_37(n),2), data_port1(index_data_port1_38(n),2), data_port2(index_data_port2_38(n),2), data_port1(index_data_port1_39(n),2), data_port2(index_data_port2_39(n),2), data_port3(index_data_port3_37(n),2), data_port4(index_data_port4_37(n),2), data_port3(index_data_port3_38(n),2), data_port4(index_data_port4_38(n),2), data_port3(index_data_port3_39(n),2), data_port4(index_data_port4_39(n),2)];
        eval([LWA_SC_name_vector ' = LWA_SC_RSSI_vector;']);   
        
        %SCFP Calibration vector for Monpoles --> Ports from #5 to #8
        Monopole_SC_name_vector = sprintf('x_%d_y_%d_n_%d_Monopole_SC', X, Y, n);
        Monopole_SC_RSSI_vector = [data_port5(index_data_port5_37(n),2), data_port6(index_data_port6_37(n),2), data_port5(index_data_port5_38(n),2), data_port6(index_data_port6_38(n),2), data_port5(index_data_port5_39(n),2), data_port6(index_data_port6_39(n),2), data_port7(index_data_port7_37(n),2), data_port8(index_data_port8_37(n),2), data_port7(index_data_port7_38(n),2), data_port8(index_data_port8_38(n),2), data_port7(index_data_port7_39(n),2), data_port8(index_data_port8_39(n),2)];
        eval([Monopole_SC_name_vector ' = Monopole_SC_RSSI_vector;']);


        %UCFP Test vector for LWA --> Ports from #1 to #4 and channels
        %unified 
        LWA_UC_name_vector = sprintf('x_%d_y_%d_n_%d_LWA_UC', X, Y, n);
        LWA_UC_RSSI_vector = [mean(data_port1_UC(v:w,2)), mean(data_port2_UC(v:w,2)), mean(data_port3_UC(v:w,2)), mean(data_port4_UC(v:w,2))];
        eval([LWA_UC_name_vector ' = LWA_UC_RSSI_vector;']);   
        
        %Vectores calibracion Monopolos
        Monopole_UC_name_vector = sprintf('x_%d_y_%d_n_%d_Monopole_UC', X, Y, n);
        Monopole_UC_RSSI_vector = [mean(data_port5_UC(v:w,2)), mean(data_port6_UC(v:w,2)), mean(data_port7_UC(v:w,2)), mean(data_port8_UC(v:w,2))];
...
%% The x_%d_y_%d_LWA_SC vectors are iterated for generate the LWA_SC_Test_Matrix
LWA_SC_Test_Matrix = zeros(1300, 12);
LWA_SC_index = 1;
for i= 1:10
    for j= 1:13
        for k= 1:10
            LWA_SC_row_vector = sprintf('x_%d_y_%d_n_%d_LWA_SC', i, j, k);
            LWA_SC_selected_vector = eval(LWA_SC_row_vector);
            LWA_SC_Test_Matrix(LWA_SC_index, :) = LWA_SC_selected_vector;
...
LWA_SC_Test_Matrix(:,1) = LWA_SC_Test_Matrix(:,1)+equalization_vector_antenna1(1);
LWA_SC_Test_Matrix(:,2) = LWA_SC_Test_Matrix(:,2)+equalization_vector_antenna1(4);
LWA_SC_Test_Matrix(:,3) = LWA_SC_Test_Matrix(:,3)+equalization_vector_antenna1(2);
LWA_SC_Test_Matrix(:,4) = LWA_SC_Test_Matrix(:,4)+equalization_vector_antenna1(5);
LWA_SC_Test_Matrix(:,5) = LWA_SC_Test_Matrix(:,5)+equalization_vector_antenna1(3);
LWA_SC_Test_Matrix(:,6) = LWA_SC_Test_Matrix(:,6)+equalization_vector_antenna1(6);

LWA_SC_Test_Matrix(:,7) = LWA_SC_Test_Matrix(:,7)+equalization_vector_antenna2(1);
LWA_SC_Test_Matrix(:,8) = LWA_SC_Test_Matrix(:,8)+equalization_vector_antenna2(4);
LWA_SC_Test_Matrix(:,9) = LWA_SC_Test_Matrix(:,9)+equalization_vector_antenna2(2);
LWA_SC_Test_Matrix(:,10) = LWA_SC_Test_Matrix(:,10)+equalization_vector_antenna2(5);
LWA_SC_Test_Matrix(:,11) = LWA_SC_Test_Matrix(:,11)+equalization_vector_antenna2(3);
LWA_SC_Test_Matrix(:,12) = LWA_SC_Test_Matrix(:,12)+equalization_vector_antenna2(6);

%Saving LWA_SC_Test_Matrix into a .mat file
save(sprintf('LWA_SC_Test_Matrix_day_%d.mat', str2num(day)),'LWA_SC_Test_Matrix');

%% The x_%d_y_%d_Monopole_UC vectors are iterated for generate the Monopole_UC_Test_Matrix
Monopole_SC_Test_Matrix = zeros(1300, 12);
Monopole_SC_index = 1;
for i= 1:10
    for j= 1:13
        for k= 1:10
            Monopole_SC_row_vector = sprintf('x_%d_y_%d_n_%d_Monopole_SC', i, j, k);
            Monopole_SC_selected_vector = eval(Monopole_SC_row_vector);
            Monopole_SC_Test_Matrix(Monopole_SC_index, :) = Monopole_SC_selected_vector;
            Monopole_SC_index = Monopole_SC_index + 1;
        end    
    end
end
%Saving Monopoles_SC_Calibration_Matrix into a .mat file
save(sprintf('Monopole_SC_Test_Matrix_day_%d.mat',str2num(day)),'Monopole_SC_Test_Matrix');

%% The x_%d_y_%d_LWA_UC vectors are iterated for generate the LWA_UC_Test_Matrix
LWA_UC_Test_Matrix = zeros(130, 4);
LWA_UC_index = 1;
for i= 1:10
    for j= 1:13
        for k= 1:10
            LWA_UC_row_vector = sprintf(['x_%d_y_%d_n_%d_LWA_UC'], i, j, k);
            LWA_UC_selected_vector = eval(LWA_UC_row_vector);
            LWA_UC_Test_Matrix(LWA_UC_index, :) = LWA_UC_selected_vector;
...
%Saving LWA_SC_Test_Matrix into a .mat file
save(sprintf('LWA_UC_Test_Matrix_day_%d.mat', str2num(day)),'LWA_UC_Test_Matrix');

%% The x_%d_y_%d_Monopole_UC vectors are iterated for generate the LWA_UC_Test_Matrix
Monopole_UC_Test_Matrix = zeros(1300, 4);
Monopole_UC_index = 1;
for i= 1:10
    for j= 1:13
        for k= 1:10
            Monopole_UC_row_vector = sprintf(['x_%d_y_%d_n_%d_Monopole_UC'], i, j, k);
            Monopole_UC_selected_vector = eval(Monopole_UC_row_vector);
            Monopole_UC_Test_Matrix(Monopole_UC_index, :) = Monopole_UC_selected_vector;
            Monopole_UC_index = Monopole_UC_index + 1;
        end    
    end
end

save(sprintf('Monopole_UC_Test_Matrix_day_%d.mat', str2num(day)),'Monopole_UC_Test_Matrix');
